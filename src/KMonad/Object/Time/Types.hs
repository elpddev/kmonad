module KMonad.Object.Time.Types
  ( Time(..)
  , HasTime(..)
  , _SystemTime, _s, _ns
  , SystemTime(..)
...
-- | Some object data-point with a timestamp
newtype Timed a = Timed { unTimed :: (Time, a) }
  deriving (Eq, Show, Functor)
makeWrapped ''Timed

instance HasTime (Timed a)   where time = _Wrapped . _1
instance HasVal  (Timed a) a where val  = _Wrapped . _2
